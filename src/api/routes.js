...
function createAPIRouter(storage, options = {}) {
  ...
  router.get('/sessions/:id/captures', (req, res) => {
    ...
    const items = captures.map((c) => ({
      ...
      breakdown: c.breakdown ? {
        ...
        system_prompts: { tokens: c.breakdown.system_prompts.tokens, percentage: c.breakdown.system_prompts.percentage, token_method: c.breakdown.system_prompts.token_method, token_confidence: c.breakdown.system_prompts.token_confidence, token_source: c.breakdown.system_prompts.token_source },
        tool_definitions: { tokens: c.breakdown.tool_definitions.tokens, percentage: c.breakdown.tool_definitions.percentage, count: c.breakdown.tool_definitions.count, token_method: c.breakdown.tool_definitions.token_method, token_confidence: c.breakdown.tool_definitions.token_confidence, token_source: c.breakdown.tool_definitions.token_source },
        tool_calls: { tokens: c.breakdown.tool_calls.tokens, percentage: c.breakdown.tool_calls.percentage, count: c.breakdown.tool_calls.count, token_method: c.breakdown.tool_calls.token_method, token_confidence: c.breakdown.tool_calls.token_confidence, token_source: c.breakdown.tool_calls.token_source },
        tool_results: { tokens: c.breakdown.tool_results.tokens, percentage: c.breakdown.tool_results.percentage, count: c.breakdown.tool_results.count, token_method: c.breakdown.tool_results.token_method, token_confidence: c.breakdown.tool_results.token_confidence, token_source: c.breakdown.tool_results.token_source },
        assistant_text: { tokens: c.breakdown.assistant_text.tokens, percentage: c.breakdown.assistant_text.percentage, token_method: c.breakdown.assistant_text.token_method, token_confidence: c.breakdown.assistant_text.token_confidence, token_source: c.breakdown.assistant_text.token_source },
        user_text: { tokens: c.breakdown.user_text.tokens, percentage: c.breakdown.user_text.percentage, messageCount: c.breakdown.user_text.messageCount, token_method: c.breakdown.user_text.token_method, token_confidence: c.breakdown.user_text.token_confidence, token_source: c.breakdown.user_text.token_source },
        thinking_blocks: { tokens: c.breakdown.thinking_blocks.tokens, percentage: c.breakdown.thinking_blocks.percentage, token_method: c.breakdown.thinking_blocks.token_method, token_confidence: c.breakdown.thinking_blocks.token_confidence, token_source: c.breakdown.thinking_blocks.token_source },
        media: { tokens: c.breakdown.media.tokens, percentage: c.breakdown.media.percentage, count: c.breakdown.media.count, token_method: c.breakdown.media.token_method, token_confidence: c.breakdown.media.token_confidence, token_source: c.breakdown.media.token_source },
  ...
  router.get('/sessions/:id/composition', (req, res) => {
    ...
    res.json({
      composition: {
        categories: [
          { name: 'System Prompts', key: 'system_prompts', tokens: b.system_prompts.tokens, percentage: b.system_prompts.percentage, color: '#6366f1', token_method: b.system_prompts.token_method, token_confidence: b.system_prompts.token_confidence, token_source: b.system_prompts.token_source },
          { name: 'Tool Definitions', key: 'tool_definitions', tokens: b.tool_definitions.tokens, percentage: b.tool_definitions.percentage, color: '#f59e0b', token_method: b.tool_definitions.token_method, token_confidence: b.tool_definitions.token_confidence, token_source: b.tool_definitions.token_source },
          { name: 'Tool Calls', key: 'tool_calls', tokens: b.tool_calls.tokens, percentage: b.tool_calls.percentage, color: '#ef4444', token_method: b.tool_calls.token_method, token_confidence: b.tool_calls.token_confidence, token_source: b.tool_calls.token_source },
          { name: 'Tool Results', key: 'tool_results', tokens: b.tool_results.tokens, percentage: b.tool_results.percentage, color: '#10b981', token_method: b.tool_results.token_method, token_confidence: b.tool_results.token_confidence, token_source: b.tool_results.token_source },
          { name: 'Assistant Text', key: 'assistant_text', tokens: b.assistant_text.tokens, percentage: b.assistant_text.percentage, color: '#f97316', token_method: b.assistant_text.token_method, token_confidence: b.assistant_text.token_confidence, token_source: b.assistant_text.token_source },
          { name: 'User Text', key: 'user_text', tokens: b.user_text.tokens, percentage: b.user_text.percentage, color: '#06b6d4', token_method: b.user_text.token_method, token_confidence: b.user_text.token_confidence, token_source: b.user_text.token_source },
          { name: 'Thinking', key: 'thinking_blocks', tokens: b.thinking_blocks.tokens, percentage: b.thinking_blocks.percentage, color: '#a855f7', token_method: b.thinking_blocks.token_method, token_confidence: b.thinking_blocks.token_confidence, token_source: b.thinking_blocks.token_source },
          { name: 'Media', key: 'media', tokens: b.media.tokens, percentage: b.media.percentage, color: '#ec4899', token_method: b.media.token_method, token_confidence: b.media.token_confidence, token_source: b.media.token_source },
        ],
  ...
  router.get('/budget/settings', (req, res) => {
    if (!storage || typeof storage.getBudgetSettings !== 'function') {
      return res.status(503).json({ error: 'Budget settings unavailable' });
    }
    const project = req.query.project ? String(req.query.project) : 'default';
    if (!isBudgetProjectAllowed(req.auth, project)) {
      return res.status(403).json({ error: 'Project not allowed by auth scope' });
    }
    const current = storage.getBudgetSettings(project);
    const fallback = resolveBudgetThresholds();
    return res.json({
      project,
      thresholds: current?.thresholds || fallback,
      source: current ? 'storage' : 'default',
      updatedAt: current?.updatedAt || null,
    });
  });

  router.get('/budget/settings/list', (req, res) => {
    if (!storage || typeof storage.listBudgetSettings !== 'function') {
      return res.status(503).json({ error: 'Budget settings unavailable' });
    }
    const items = storage.listBudgetSettings().filter((item) => isBudgetProjectAllowed(req.auth, item.project));
    return res.json({ items });
  });

  router.get('/budget/settings/export', (req, res) => {
    if (!storage || typeof storage.getBudgetSettings !== 'function') {
      return res.status(503).json({ error: 'Budget settings unavailable' });
    }
    const project = req.query.project ? String(req.query.project) : 'default';
    ...
    const settings = storage.getBudgetSettings(project);
    if (settings) {
      return res.json({
        project,
    ...
    return res.json({
      project,
      thresholds: resolveBudgetThresholds(),
      updatedAt: null,
      source: 'default',
      exportedAt: Date.now(),
    });
  ...
  router.post('/budget/settings', requireRole('editor'), (req, res) => {
    if (!storage || typeof storage.upsertBudgetSettings !== 'function') {
      return res.status(503).json({ error: 'Budget settings unavailable' });
    }
    const project = req.body?.project ? String(req.body.project) : 'default';
    if (!isBudgetProjectAllowed(req.auth, project)) {
      return res.status(403).json({ error: 'Project not allowed by auth scope' });
    }
    const thresholds = req.body?.thresholds || {};
    const saved = storage.upsertBudgetSettings(project, thresholds);
    return res.json({ ok: true, project, thresholds: saved.thresholds, updatedAt: saved.updatedAt, source: 'storage' });
  });

  router.post('/budget/settings/import', requireRole('editor'), (req, res) => {
    if (!storage || typeof storage.upsertBudgetSettings !== 'function') {
      return res.status(503).json({ error: 'Budget settings unavailable' });
    }
    const imported = normalizeBudgetImportPayload(req.body);
    if (!imported.ok) return res.status(400).json({ error: imported.error });
    if (!isBudgetProjectAllowed(req.auth, imported.project)) {
      return res.status(403).json({ error: 'Project not allowed by auth scope' });
    }
    const saved = storage.upsertBudgetSettings(imported.project, imported.thresholds);
    return res.json({
      ok: true,
  ...
  router.post('/budget/settings/reset', requireRole('editor'), (req, res) => {
    if (!storage || typeof storage.clearBudgetSettings !== 'function') {
      return res.status(503).json({ error: 'Budget settings unavailable' });
    }
    const project = req.body?.project ? String(req.body.project) : 'default';
    if (!isBudgetProjectAllowed(req.auth, project)) {
    ...
    const cleared = storage.clearBudgetSettings(project);
    return res.json({ ok: true, project, cleared: Boolean(cleared), source: 'storage' });
  });
...
function isBudgetProjectAllowed(auth, project) {
  if (!auth) return true;
  const projects = Array.isArray(auth.projects) ? auth.projects : [];
  if (projects.length === 0) return true;
  return projects.includes(String(project || 'default'));
}
...
function normalizeBudgetImportPayload(body) {
  const payload = body && typeof body === 'object' ? (body.payload && typeof body.payload === 'object' ? body.payload : body) : null;
  if (!payload) return { ok: false, error: 'Budget import payload must be an object' };
  const project = String(payload.project || body.project || 'default').trim() || 'default';
  const thresholds = payload.thresholds && typeof payload.thresholds === 'object' ? payload.thresholds : null;
  if (!thresholds) return { ok: false, error: 'Budget import payload must include thresholds' };
  return {
    ok: true,
    project,
    thresholds,
    source: payload.source || body.source || 'import',
  };
}
